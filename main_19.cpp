...
const int N = 210;
// 20
//  double w_good_val = 0.24878927603257966, w_good_dis = 0.4489616886811306;
//  double w_boat_speed = 0.20687594986940377, w_boat_size = 0.7619890604920266, w_boat_transport = 0.18864159386524254;
//  double w_berth_fill = 0.36248798996094883, w_berth_dis = 0.07342531329936554, w_berth_arrive = 0.6512693518015417;
//  double w_near_val = 0.2080204021425986, w_near_dis = 0.07803848202324361;
//  double max_near_dis = 100;
double w_good_val = 0.24878927603257966, w_good_dis = 0.4489616886811306;
double w_boat_speed = 0.20687594986940377, w_boat_size = 0.7619890604920266, w_boat_transport = 0.18864159386524254;
double w_berth_fill = 0.46248798996094883, w_berth_dis = 0.07342531329936554, w_berth_arrive = 0.6812693518015417;
double w_near_val = 0.2080204021425986, w_near_dis = 0.07803848202324361;
double max_near_dis = 100;
// double w_good_val = 0.04971526662949956, w_good_dis =  0.932168202660938;//, w_good_disappear = 0.04765758497181795;
// double w_boat_speed = 0.6227319510130915, w_boat_size =  0.6488801820055493, w_boat_transport = 0.24768019814398695;
// double w_berth_fill = 0.37853652991502806, w_berth_dis = 0.11507858178470379, w_berth_arrive = 0.8792027324434619;
// double w_near_val = 0.2830486889710963, w_near_dis = 0.03904869240425802;
...
int Input(int zhen)
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        if (boat[i].status == 1 && boat[i].flag == 0)
        {
            ...
            for (int t = 0; t < berth_num; t++)
            {
                if (vis_berth[t] == 0)
                {
                    ...
                    if (zhen >= 13000)
                    {
                        w = min(((15000 - 13000) - berth[t].transport_time * 2) * berth[t].loading_speed, berth[t].size);
                        w = min(w, boat_capacity * 1.0);
                    }
                    else if (zhen >= 5000)
                    {
                        w = (berth[t].size * 1.0 / boat_capacity) * (w_boat_size + 1) +
        ...
        {
            // if (zhen>5000&&zhen<=5100&&ship_run[i]==0)
            // {
            //     ship_run[i]=1;
            //     printf("go %d\n", i);
            //     boat[i].num = boat_capacity;
            ...
            // }
            // 如果时间不够
            if (14950 - berth[boat[i].berth].transport_time - zhen < 0)
            {
                printf("go %d\n", i);
                boat[i].num = boat_capacity;
                vis_berth[boat[i].berth] = 0;
...
void bfs_noway(int x, int y, int zhen, int id)
{
    ...
    while (!q.empty())
    {
        ...
        int dis = distance.front();
        distance.pop();
        if ((ch[now.first][now.second] == 'B' || ch[now.first][now.second] == '.') &&
            (now.first != x || now.second != y))
        {
            if (now.first == last_robot_vis[id][0] && now.second == last_robot_vis[id][1])
                continue;
            pos = now;
            break;
        }
...
void bfs_berth(int x, int y, int zhen, int id)
{
    ...
    for (int i = 0; i < berth_num; i++)
    {
        ...
        if (zhen >= 13000)
        {
            search_berth[++search_size].x = -10000000 * vis_berth[i] - 1000000 * berth[i].arrive - num / 1000 * w_berth_fill;
        }
        // else if (zhen <= 5000)
        // {
        //     search_berth[++search_size].x =
        //         -berth[i].arrive * (w_berth_arrive+5) +
        //         pre_dis_berth[i][x][y] * w_berth_dis - berth[i].near_val / 100 * w_near_val;
        // }
        else
        {
            search_berth[++search_size].x =
                -num / 1000 * w_berth_fill -
